...
for (const n of nodes) if (n.keyMoment && !reachable.has(n.id)) err(`keyMoment 节点 ${n.id} 不可达`);
...
const result = { file, errors, warns, ok: errors.length === 0,
  stats: { nodes: nodes.length, chapters: chapters.size, characters: novel.characters?.length || 0, endings: novel.endings?.length || 0, lorebook: novel.lorebook?.length || 0, reachable: reachable.size } };
