...
function gmSystemPrompt(novel, state) {
  ...
  return [
    ...
    `[对话响应（重要）] 1.玩家的输入若是对某人说话（提问/挑衅/搭讪/威胁/交谈），replies 里必须有该角色对这句话的直接回应——回答问题、接话、反驳或顶撞，内容要承接玩家话里的具体内容，禁止答非所问、禁止顾左右而言他。2.多人同场时至少一名在场角色回应。3.玩家独处（无人在场）时：若有系统/器灵/识海类角色（如"系统"）则由它回应；否则用 narration 演出环境对玩家言行的反馈，replies 可为空。4.mind 必须回应玩家本轮的言行——玩家的话让他动摇、恼怒、心虚或起疑，心声写他此刻真实的想法，常与嘴上说的相反（口是心非）。`,
    `[演出·旁白与台词界限] narration=第三人称旁白，演出行动过程/场景变化/他人可见的反应，禁止出现任何人的直接台词；replies=在场NPC的台词，每条≤20字、每轮1-3条、由不同角色说出，禁止夹带动作描写（动作放narration）；mind=某NPC第一人称真实心声≤25字，绝不写玩家的心声；禁emoji；NPC不能替玩家解决问题。`,
